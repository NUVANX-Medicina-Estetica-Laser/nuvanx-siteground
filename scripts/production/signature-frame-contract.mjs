 ...
 * 2. Governed frame: Exactly ONE outer global `nvx-brand-page` ancestor (restricted to
 *    <div> or <article>) containing an inner renderer root (<article> or <div>) with
 *    `nvx-brand-page--signature` and `nvx-brand-page__renderer-root` after
 *    `nvx_remove_redundant_inner_brand_page_class` normalizes redundant classes.
 */
export function hasValidSignatureFrame(html) {
  ...
  while ((match = tagRegex.exec(cleanHtml)) !== null) {
    ...
    if (tagName === 'div' || tagName === 'article') {
      ...
      const brandPageAncestors = stack.filter((parent) => parent.classes.has('nvx-brand-page'));
      const totalBrandPageAncestors = brandPageAncestors.length;
      const hasSingleValidFrameAncestor =
        totalBrandPageAncestors === 1 && (brandPageAncestors[0].tag === 'div' || brandPageAncestors[0].tag === 'article');

      // Form 1: Standalone root (requires both classes and NO outer nvx-brand-page frame ancestor)
      if (hasBrandPage && hasSignature && totalBrandPageAncestors === 0) {
        foundValidStandalone = true;
      }

      // Form 2: Governed frame (requires signature + renderer-root under exactly ONE div/article nvx-brand-page ancestor)
