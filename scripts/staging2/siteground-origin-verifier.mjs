...
export function createSiteGroundOriginVerifier({
  ...
  function verify(route) {
    ...
    const remoteScript = [
      ...
      'trap cleanup EXIT',
      'result="$(curl -4 -sS --connect-timeout 10 --max-time 30 -A \'Mozilla/5.0 NUVANX-Origin-Verification/2.1\' -H \'Accept: text/html,application/xhtml+xml\' -H "Host: ${EXPECTED_HOST}" -D "$headers" -o "$body" -w \'%{http_code}|%{url_effective}\' "${base_url}${ROUTE}")"',
      'code="${result%%|*}"',
      'effective="${result#*|}"',
      'test "$code" = \'200\'',
      'case "$effective" in "http://localhost/"*|"http://localhost") ;; *) echo "ORIGIN_VERIFY_FAIL route=$ROUTE final=$effective" >&2; exit 1 ;; esac',
  ...
  function fetchHtml(route) {
    ...
    const remoteScript = [
      ...
      'cleanup() { rm -f "$headers" "$body"; }',
      'trap cleanup EXIT',
      'result="$(curl -4 -sS --connect-timeout 10 --max-time 30 -A \'Mozilla/5.0 NUVANX-Origin-A11y/1.1\' -H \'Accept: text/html,application/xhtml+xml\' -H "Host: ${EXPECTED_HOST}" -D "$headers" -o "$body" -w \'%{http_code}|%{url_effective}\' "${base_url}${ROUTE}")"',
      'code="${result%%|*}"',
      'effective="${result#*|}"',
      'test "$code" = \'200\'',
      'case "$effective" in "http://localhost/"*|"http://localhost") ;; *) echo "ORIGIN_HTML_FAIL route=$ROUTE final=$effective" >&2; exit 1 ;; esac',
