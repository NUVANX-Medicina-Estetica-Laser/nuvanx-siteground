...
export function createSiteGroundOriginVerifier({
  ...
  function verify(route) {
    ...
    const remoteScript = [
      ...
      'set +e',
      'result="$(curl -4 -k -sS --connect-timeout 10 --max-time 30 --resolve "${EXPECTED_HOST}:443:127.0.0.1" --proto \'=https\' -A \'Mozilla/5.0 NUVANX-Origin-Verification/2.2\' -H \'Accept: text/html,application/xhtml+xml\' -H \'Cache-Control: no-cache\' -D "$headers" -o "$body" -w \'%{http_code}|%{url_effective}|%{remote_ip}\' "$origin_url")"',
      'curl_rc=$?',
      'set -e',
  ...
  function fetchHtml(route) {
    ...
    const remoteScript = [
      ...
      'set +e',
      'result="$(curl -4 -k -sS --connect-timeout 10 --max-time 30 --resolve "${EXPECTED_HOST}:443:127.0.0.1" --proto \'=https\' -A \'Mozilla/5.0 NUVANX-Origin-A11y/1.2\' -H \'Accept: text/html,application/xhtml+xml\' -H \'Cache-Control: no-cache\' -D "$headers" -o "$body" -w \'%{http_code}|%{url_effective}|%{remote_ip}\' "$origin_url")"',
      'curl_rc=$?',
      'set -e',
      '[[ "$curl_rc" -eq 0 ]] || fail_origin "curl_exit_$curl_rc"',
      'code="$(printf \'%s\' "$result" | cut -d\'|\' -f1)"',
