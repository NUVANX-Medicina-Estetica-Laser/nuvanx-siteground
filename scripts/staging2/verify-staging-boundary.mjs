...
function verifyViaSiteGroundOrigin(route) {
  ...
  const remoteScript = [
    ...
    '    result="$(curl -4 -k -sS --connect-timeout 10 --max-time 30 --resolve "${EXPECTED_HOST}:443:${_fallback_ip}" --proto \'=https\' -A \'NUVANX-Staging-Origin-Boundary/1.4\' -H \'Accept: text/html,application/xhtml+xml\' -H \'Cache-Control: no-cache\' -b \'wpSGCacheBypass=1\' -D "$headers" -o "$body" -w \'%{http_code}|%{url_effective}|%{remote_ip}\' "$origin_url")"',
    '    curl_rc=$?',
    '  fi',
