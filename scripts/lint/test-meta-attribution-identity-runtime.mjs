...
function run(href, { localStorage: providedLocalStorage = null, cookie = '' } = {}) {
  ...
  return {
    contract: window.NUVANXAttributionContract,
    localStorage,
  };
}

const metaRun = run('https://nuvanx.com/madrid/valoracion/?fbclid=MetaClick123');
const meta = metaRun.contract.getConversionTouch();
...
const googleRun = run('https://nuvanx.com/madrid/valoracion/?gclid=GoogleClick123');
const google = googleRun.contract.getConversionTouch();
...
const existingConversion = {
  ...
  source: 'google',
  medium: 'cpc',
  gclid: 'ExistingGoogleClick',
  timestamp: new Date().toISOString(),
  expires_at: Date.now() + 60_000,
};
const sharedStorage = storage({
  nvx_conversion_touch: JSON.stringify(existingConversion),
});
...
assert.equal(malformedConversion.source, 'google', 'Existing valid acquisition source must survive malformed Meta evidence');
assert.equal(malformedConversion.gclid, 'ExistingGoogleClick');
assert.notEqual(malformedFirst?.channel, 'paid_social', 'Malformed fbclid must not create paid-social attribution');
assert.notEqual(malformedFirst?.source, 'meta', 'Malformed fbclid must not infer Meta as source');
assert.equal(malformedFirst?.fbclid, undefined, 'Malformed fbclid must never enter the stored touch');

const oversizedSuffix = 'A'.repeat(500);
const oversizedFbp = `fb.1.1788140000000.${oversizedSuffix}`;
const oversizedCookieRun = run('https://nuvanx.com/madrid/valoracion/', { cookie: `_fbp=${oversizedFbp}` });
const oversizedCookieTouch = oversizedCookieRun.contract.getConversionTouch();
assert.equal(oversizedCookieTouch.fbp, undefined, 'Complete FBP values over 512 characters must be rejected');
