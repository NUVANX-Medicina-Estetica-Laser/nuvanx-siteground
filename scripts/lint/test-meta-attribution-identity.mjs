...
assert.match(runtime, /if \(urlClickSignal\(url, 'fbclid'\)\) return 'paid_social'/,
  'A bare fbclid must classify as paid social without requiring UTM decoration');
...
assert.match(relay, /nvx_hubspot_secure_post_value\( 'fbclid', 512 \)/,
  'Relay must accept fbclid only from the validated first-party request');
assert.match(relay, /\$_COOKIE\[ \$cookie_name \]/,
  'Relay may fall back to real first-party request cookies for FBC/FBP');
...
console.log('META_ATTRIBUTION_IDENTITY=PASS contract=consent-aware fbclid=1 fbc=1 fbp=real-cookie-only');
