...
assert.ok(routing.includes("'#' === $relative_url[0]"), 'hash-valued metadata must suppress policy-label fallback');
assert.ok(routing.includes('$has_unresolved_href = 1 === preg_match('), 'label-only unresolved hrefs must enter the canonical finalizer');
assert.ok(routing.includes("( '#' === $href || '' === $href ) && '' !== $destination"), 'unresolved policy hrefs must not require data-relative_url to canonicalize');
...
console.log('COMPLIANZ_POLICY_ROUTING_STATIC=PASS owner=canonical metadata=authoritative label_fallback=covered translated_hash_links=covered');
