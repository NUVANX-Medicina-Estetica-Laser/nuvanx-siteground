...
nvx_p0_clinic_assert( '' !== $template_source && '' !== $business_source && '' !== $identity_source, 'OWNERS_READABLE' );
...
nvx_p0_clinic_assert( ! str_contains( $template_source, 'get_post_field(' ), 'TEMPLATE_PERSISTED_SLUG_NOT_IDENTITY_OWNER' );
nvx_p0_clinic_assert( str_contains( $template_source, "get_query_var( 'nvx_clinic_key'" ), 'TEMPLATE_CONSUMES_ROUTED_KEY' );
nvx_p0_clinic_assert( str_contains( $template_source, 'nvx_current_clinic_landing_key()' ), 'TEMPLATE_HAS_EXACT_FALLBACK_RESOLVER' );
nvx_p0_clinic_assert( 1 === substr_count( $business_source . $identity_source, "add_filter( 'template_include'" ), 'SINGLE_TEMPLATE_INCLUDE_OWNER' );
nvx_p0_clinic_assert( str_contains( $business_source, "add_filter( 'template_include', 'nvx_clinic_template_fail_closed', 99 )" ), 'BUSINESS_CONFIG_OWNS_TEMPLATE_ROUTING' );
nvx_p0_clinic_assert( ! str_contains( $identity_source, 'template_include' ), 'SCHEMA_FENCE_NOT_TEMPLATE_OWNER' );

echo 'CLINIC_TEMPLATE_ROUTING=PASS source=clinics-json exact=2 substring=blocked nested=blocked template=fail_closed single_owner=business-config' . PHP_EOL;
