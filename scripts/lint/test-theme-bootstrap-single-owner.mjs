...
// can remain during retirement, but an unregistered route must receive an
// explicit non-empty sentinel before nvx_get_page_owner() could consult them.
...
console.log(`THEME_BOOTSTRAP_SINGLE_OWNER=PASS modules=${manifest.length} request_snapshot=early schema_package=single_owner page_owner=canonical-fail-closed`);
