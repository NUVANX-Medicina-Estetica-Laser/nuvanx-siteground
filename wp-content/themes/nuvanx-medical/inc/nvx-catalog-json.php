...
function nvx_catalog_faq_index_by_id( array $catalog, string $faq_id ): ?int {
	if (
		'' === $faq_id
		|| ! isset( $catalog['faq'] )
		|| ! is_array( $catalog['faq'] )
		|| ! isset( $catalog['faq']['items'] )
		|| ! is_array( $catalog['faq']['items'] )
	) {
		return null;
	}
...
function nvx_catalog_price_destination_valid( array $catalog, string $block_key, string $faq_id ): bool {
	$faq_index = nvx_catalog_faq_index_by_id( $catalog, $faq_id );

	return isset( $catalog[ $block_key ] )
		&& is_array( $catalog[ $block_key ] )
		&& array_key_exists( 'body', $catalog[ $block_key ] )
		&& null !== $faq_index
		&& isset( $catalog['faq']['items'][ $faq_index ] )
		&& is_array( $catalog['faq']['items'][ $faq_index ] )
		&& array_key_exists( 'a', $catalog['faq']['items'][ $faq_index ] );
}

/**
 * Neutralize governed FAQ pricing copy by stable identity.
 *
 * If the identity is missing or duplicated, neutralize any price-looking FAQ
 * answer as a defensive fail-closed fallback. This prevents editorial reorder
 * or identity drift from leaving an old numeric answer public.
 */
function nvx_catalog_suppress_faq_price_copy( array &$catalog, string $faq_id, string $neutral ): void {
	if (
		! isset( $catalog['faq'] )
		|| ! is_array( $catalog['faq'] )
		|| ! isset( $catalog['faq']['items'] )
	...
	foreach ( $catalog['faq']['items'] as $index => $item ) {
		...
		if ( $faq_id === (string) ( $item['id'] ?? '' ) && array_key_exists( 'a', $item ) ) {
...
/** Neutralize one structured investment/planning + FAQ price destination when present. */
function nvx_catalog_suppress_structured_price_copy(
	array &$catalog,
	string $block_key,
	string $faq_id,
	string $neutral
): void {
	if ( isset( $catalog[ $block_key ] ) && is_array( $catalog[ $block_key ] ) && array_key_exists( 'body', $catalog[ $block_key ] ) ) {
		$catalog[ $block_key ]['body'] = $neutral;
	}

	nvx_catalog_suppress_faq_price_copy( $catalog, $faq_id, $neutral );
}
...
function nvx_catalog_suppress_price_copy( array $catalog, string $safe_name, array $config ): array {
	...
	if ( 'endolift-page.json' === $safe_name ) {
		$inv_key = (string) ( $config['endolift']['investment_key'] ?? 'investment' );
		$faq_id  = (string) ( $config['endolift']['price_faq_id'] ?? 'pricing' );
		nvx_catalog_suppress_structured_price_copy( $catalog, $inv_key, $faq_id, $neutral );
	} elseif ( 'exion-page.json' === $safe_name ) {
		$inv_key = (string) ( $config['exion']['investment_key'] ?? 'investment' );
		$faq_id  = (string) ( $config['exion']['price_faq_id'] ?? 'pricing' );
		nvx_catalog_suppress_structured_price_copy( $catalog, $inv_key, $faq_id, $neutral );
	} elseif ( 'endolaser-page.json' === $safe_name ) {
		$plan_key = (string) ( $config['endolaser']['planning_key'] ?? 'planning' );
		$faq_id   = (string) ( $config['endolaser']['price_faq_id'] ?? 'pricing' );
		nvx_catalog_suppress_structured_price_copy( $catalog, $plan_key, $faq_id, $neutral );
	} elseif ( 'aesthetic-medicine-page.json' === $safe_name ) {
		// The whole treatment-card price field is tariff-governed. Neutralize every
		// card first so renamed, duplicated or unexpected identities cannot retain
		// stale numeric editorial copy. Canonical identities are restored later.
		if ( isset( $catalog['treatments'] ) && is_array( $catalog['treatments'] ) ) {
			foreach ( $catalog['treatments'] as $index => $treatment ) {
				if ( is_array( $treatment ) ) {
					$catalog['treatments'][ $index ]['price'] = $neutral;
				}
			}
		}
	} elseif ( 'laser-co2-page.json' === $safe_name ) {
		$inv_key = (string) ( $config['laser_co2']['investment_key'] ?? 'investment' );
		$faq_id  = (string) ( $config['laser_co2']['price_faq_id'] ?? 'pricing' );
		nvx_catalog_suppress_structured_price_copy( $catalog, $inv_key, $faq_id, $neutral );
	}
 ...
 * restored only when every tariff required by that public block and every
 * editorial destination required to render it are complete.
 */
function nvx_catalog_apply_tariff_truth( array $catalog, string $safe_name, ?array $config = null ): array {
	...
	if ( 'endolift-page.json' === $safe_name ) {
		...
		$faq_id    = (string) ( $config['endolift']['price_faq_id'] ?? 'pricing' );
		$faq_index = nvx_catalog_faq_index_by_id( $catalog, $faq_id );
		if ( ! nvx_catalog_price_destination_valid( $catalog, $inv_key, $faq_id ) || null === $faq_index ) {
			nvx_catalog_log_error( 'Endolift® price hydration failed closed: editorial price destination malformed or pricing FAQ identity invalid.' );
			return $catalog;
		}
		...
		$mandibula = nvx_catalog_tariff_display_price( $tariffs, 'Endolift®', 'marcacion_mandibular' );
		...
		$catalog[ $inv_key ]['body'] = sprintf(
			__( 'El plan y presupuesto de Endolift® se determinan tras la valoración médica presencial en Chamberí o Salamanca–Goya. Tarifas de referencia: desde %1$s (ojeras), %2$s (papada), %3$s (marcación mandibular), %4$s (cuello). Combos frecuentes como papada+cuello (%5$s) o full face (%6$s) se valoran según indicación. El presupuesto definitivo se documenta tras valoración anatómica presencial. El procedimiento se realiza en 1 sola sesión en la mayoría de indicaciones, con control evolutivo a los 3 y 6 meses. Cada tratamiento incluye:', 'nuvanx-medical' ),
			$ojeras,
			$papada,
			$mandibula,
			$cuello,
		...
		$catalog['faq']['items'][ $faq_index ]['a'] = sprintf(
			__( 'La tarifa de referencia parte desde %1$s (ojeras). Papada: %2$s. Marcación mandibular: %3$s. El presupuesto definitivo se documenta tras valoración anatómica presencial.', 'nuvanx-medical' ),
			$ojeras,
			$papada,
			$mandibula
		);
	...
	if ( 'exion-page.json' === $safe_name ) {
		...
		$faq_index = nvx_catalog_faq_index_by_id( $catalog, $faq_id );
		if ( ! nvx_catalog_price_destination_valid( $catalog, $inv_key, $faq_id ) || null === $faq_index ) {
			nvx_catalog_log_error( 'EXION® price hydration failed closed: editorial price destination malformed or pricing FAQ identity invalid.' );
			return $catalog;
		}
	...
	if ( 'endolaser-page.json' === $safe_name ) {
		...
		$faq_id    = (string) ( $config['endolaser']['price_faq_id'] ?? 'pricing' );
		$faq_index = nvx_catalog_faq_index_by_id( $catalog, $faq_id );
		if ( ! nvx_catalog_price_destination_valid( $catalog, $plan_key, $faq_id ) || null === $faq_index ) {
			nvx_catalog_log_error( 'Endoláser price hydration failed closed: editorial price destination malformed or pricing FAQ identity invalid.' );
			return $catalog;
		}
		...
		$muslos   = nvx_catalog_tariff_display_price( $tariffs, 'Endolift®', 'muslos_internos' );
		...
		$catalog[ $plan_key ]['body'] = sprintf(
			__( 'El presupuesto se calcula por zona o combinación de zonas según el tarifario vigente y se documenta tras la valoración médica presencial. Tarifas de referencia vigentes: desde %1$s (rodillas), %2$s (brazos), %3$s (cara interna de muslos), %4$s (flancos) y %5$s (abdomen completo). Procedimiento en 1 sesión única con medición ecográfica previa y revisiones clínicas a 4 semanas, 3 y 6 meses. Incluye pauta de compresión post-procedimiento.', 'nuvanx-medical' ),
			$rodillas,
			$brazos,
			$muslos,
			$flancos,
	...
	$faq_index = nvx_catalog_faq_index_by_id( $catalog, $faq_id );
	if ( ! nvx_catalog_price_destination_valid( $catalog, $inv_key, $faq_id ) || null === $faq_index ) {
		nvx_catalog_log_error( 'Laser CO2 price hydration failed closed: editorial price destination malformed or pricing FAQ identity invalid.' );
		return $catalog;
	}
