...
/** Compatibility alias for the shared server-side consent authority. */
...
function nvx_valoracion_emit_direct_success(): void {
	...
	$event = array(
		'event' => 'nvx_conversion_signal', 'nvx_event_name' => 'generate_lead', 'page_path' => '/gracias/',
		'event_source' => 'nuvanx_theme', 'form_id' => $form_id, 'form_context' => 'valoracion',
		'lead_source' => 'first_party_form', 'form_event_source' => 'server_redirect',
	);
...
function nvx_valoracion_maybe_handle_direct_submit(): void {
	...
	if ( ! isset( $_POST['nvx_valoracion_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( (string) $_POST['nvx_valoracion_nonce'] ) ), 'nvx_valoracion_submit' ) ) {
		nvx_valoracion_log_outcome( 'FAILURE', 'nonce', 0, array() ); wp_safe_redirect( $fail ); exit;
	}
	...
	if ( ! $privacy || ! is_email( $email ) || nvx_valoracion_name_length( $firstname ) < 2 || nvx_valoracion_name_length( $lastname ) < 2 || strlen( $phone ) < 7 || nvx_valoracion_name_length( $message ) < 10 ) {
		nvx_valoracion_log_outcome( 'FAILURE', 'validation', 0, array() ); wp_safe_redirect( $fail ); exit;
	}
	...
	if ( $marketing_consent && isset( $_COOKIE['hubspotutk'] ) ) {
		...
		if ( '' !== $hutk ) { $context['hutk'] = $hutk; $hutk_present = 'yes'; }
	}
	...
	$qa_context = array(
		'portal_id' => $portal, 'form_id' => $form, 'pageUri_hash' => substr( md5( $page_uri ), 0, 8 ),
		'consent' => $marketing_consent ? 'granted' : 'denied', 'hutk_present' => $hutk_present,
...
function nvx_valoracion_forward_to_hubspot( array $fields, array $context ): array {
	...
	$body = wp_json_encode( array(
		...
		'legalConsentOptions' => array( 'consent' => array( 'consentToProcess' => true, 'text' => 'Al facilitar tus datos aceptas la Política de privacidad y el tratamiento de mis datos para gestionar esta solicitud.' ) ),
	) );
