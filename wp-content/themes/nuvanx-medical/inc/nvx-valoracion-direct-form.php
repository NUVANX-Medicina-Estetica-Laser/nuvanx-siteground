...
function nvx_valoracion_direct_form_markup(): string {
	...
	$identity_fields = array(
		array( 'id' => 'firstname', 'label' => esc_html__( 'Nombre', 'nuvanx-medical' ), 'autocomplete' => 'given-name', 'maxlength' => 80 ),
		array( 'id' => 'lastname', 'label' => esc_html__( 'Apellidos', 'nuvanx-medical' ), 'autocomplete' => 'family-name', 'maxlength' => 120 ),
	);
	...
	$html .= '<p class="nvx-valoracion-direct-form__field"><label for="nvx-valoracion-phone">' . esc_html__( 'Teléfono', 'nuvanx-medical' ) . '</label><input class="hs-input" id="nvx-valoracion-phone" name="phone" type="tel" autocomplete="tel" inputmode="tel" minlength="7" maxlength="20" required' . ( $error ? ' aria-invalid="true" aria-describedby="nvx-valoracion-error"' : '' ) . '></p>';
	$html .= '<p class="nvx-valoracion-direct-form__field"><label for="nvx-valoracion-email">' . esc_html__( 'Email', 'nuvanx-medical' ) . '</label><input class="hs-input" id="nvx-valoracion-email" name="email" type="email" autocomplete="email" maxlength="120" required' . ( $error ? ' aria-invalid="true" aria-describedby="nvx-valoracion-error"' : '' ) . '></p>';
	$html .= '<p class="nvx-valoracion-direct-form__field"><label for="nvx-valoracion-message">' . esc_html__( 'Qué quieres valorar', 'nuvanx-medical' ) . '</label><textarea class="hs-input" id="nvx-valoracion-message" name="message" rows="4" minlength="10" maxlength="2000" required' . ( $error ? ' aria-invalid="true" aria-describedby="nvx-valoracion-error"' : '' ) . '></textarea></p>';

	$html .= '<p class="nvx-valoracion-direct-form__consent"><label for="nvx-valoracion-privacy"><input id="nvx-valoracion-privacy" name="privacy" type="checkbox" value="1" required' . ( $error ? ' aria-invalid="true" aria-describedby="nvx-valoracion-error"' : '' ) . '> ';
	$html .= sprintf(
	...
	$managed_hidden = array(
		'nvx_first_source', 'nvx_first_medium', 'nvx_first_campaign_id', 'nvx_first_referrer_domain', 'nvx_first_landing_url', 'nvx_first_timestamp', 'nvx_first_channel',
		'nvx_conversion_channel', 'nvx_conversion_source', 'nvx_conversion_medium', 'nvx_conversion_campaign_id', 'nvx_conversion_landing_url', 'nvx_conversion_timestamp',
		'nvx_landing_url', 'nvx_attribution_captured_at', 'nvx_attribution_expires_at',
	);
	...
	$html .= '<button type="submit" class="nvx-brand-btn nvx-btn--primary nvx-valoracion-direct-form__submit">' . esc_html__( 'Solicitar valoración médica', 'nuvanx-medical' ) . '</button></form>';
	return $html;
}
...
function nvx_valoracion_name_length( string $value ): int {
	...
	if ( function_exists( 'iconv_strlen' ) ) {
		...
		if ( false !== $iconv_length ) return (int) $iconv_length;
	}
...
function nvx_valoracion_lead_id(): string {
	...
	if ( '' !== $posted && nvx_valoracion_is_uuid_v4( $posted ) ) return $posted;
...
function nvx_valoracion_append_field( array &$fields, string $name, string $value ): void {
	if ( '' === $value ) return;
	$fields[] = array( 'objectTypeId' => '0-1', 'name' => $name, 'value' => $value );
}

function nvx_valoracion_has_marketing_consent(): bool {
	return function_exists( 'nvx_marketing_consent_granted' ) && nvx_marketing_consent_granted();
}
...
function nvx_valoracion_log_outcome( string $outcome, string $reason = '', int $status = 0, array $qa_context = array() ): void {
	...
	if ( ! in_array( $outcome, $allowed_outcomes, true ) ) return;
	$line = 'NVX_VALORACION_' . $outcome;
	if ( 'FAILURE' === $outcome && in_array( $reason, $allowed_reasons, true ) ) $line .= ' reason=' . $reason;
	if ( $status > 0 ) $line .= ' status=' . (int) $status;
	if ( ! empty( $qa_context ) ) {
...
function nvx_valoracion_direct_success_redirect_url(): string {
	try {
		...
		if ( ! set_transient( 'nvx_success_' . $hash, 1, 10 * MINUTE_IN_SECONDS ) ) return home_url( '/gracias/' );
...
function nvx_valoracion_prepare_direct_success(): void {
	...
	$is_thank_you = function_exists( 'nvx_theme_thank_you_page_slugs' ) ? is_page( nvx_theme_thank_you_page_slugs() ) : is_page( 'gracias' );
	$token = nvx_valoracion_direct_success_token();
	if ( ! $is_thank_you || '' === $token ) return;
	if ( ! defined( 'DONOTCACHEPAGE' ) ) define( 'DONOTCACHEPAGE', true );
	nocache_headers();
	if ( 1 !== preg_match( '/^[a-f0-9]{64}$/D', $token ) ) return;
	$key = 'nvx_success_' . hash( 'sha256', $token );
	if ( false === get_transient( $key ) || ! delete_transient( $key ) ) return;
...
function nvx_valoracion_emit_direct_success(): void {
	if ( empty( $GLOBALS['nvx_valoracion_direct_success_ready'] ) ) return;
	$GLOBALS['nvx_valoracion_direct_success_ready'] = false;
	$form_id = function_exists( 'nvx_hubspot_secure_form_id' ) ? nvx_hubspot_secure_form_id() : '';
	if ( '' === $form_id ) { error_log( 'NVX_VALORACION_CONVERSION_SUPPRESSED reason=hubspot_identity_not_configured' ); return; }
	$event = array(
		'event' => 'nvx_conversion_signal', 'nvx_event_name' => 'generate_lead', 'page_path' => '/gracias/', 'event_source' => 'nuvanx_theme',
		'form_id' => $form_id, 'form_context' => 'valoracion', 'lead_source' => 'first_party_form', 'form_event_source' => 'server_redirect',
	);
	$payload = wp_json_encode( $event, JSON_UNESCAPED_SLASHES );
	if ( is_string( $payload ) ) wp_print_inline_script_tag( 'window.dataLayer=window.dataLayer||[];window.dataLayer.push(' . $payload . ');' );
}
add_action( 'wp_head', 'nvx_valoracion_emit_direct_success', 5 );

function nvx_valoracion_maybe_handle_direct_submit(): void {
	if ( 'POST' !== strtoupper( (string) ( $_SERVER['REQUEST_METHOD'] ?? '' ) ) || empty( $_POST['nvx_valoracion_submit'] ) ) return;
	$referer = wp_get_referer();
	$back = is_string( $referer ) && '' !== $referer ? $referer : home_url( '/madrid/valoracion/' );
	$fail = add_query_arg( 'valoracion', 'error', $back );
	if ( ! isset( $_POST['nvx_valoracion_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( (string) $_POST['nvx_valoracion_nonce'] ) ), 'nvx_valoracion_submit' ) ) { nvx_valoracion_log_outcome( 'FAILURE', 'nonce', 0, array() ); wp_safe_redirect( $fail ); exit; }
	$honeypot = isset( $_POST['nvx_company'] ) ? trim( (string) wp_unslash( $_POST['nvx_company'] ) ) : '';
	if ( '' !== $honeypot ) { wp_safe_redirect( $fail ); exit; }
	$ip = sanitize_text_field( wp_unslash( (string) ( $_SERVER['REMOTE_ADDR'] ?? '0' ) ) );
	$rate_key = 'nvx_val_rl_' . hash( 'sha256', $ip );
	$hits = (int) get_transient( $rate_key );
	if ( $hits >= 5 ) { nvx_valoracion_log_outcome( 'FAILURE', 'rate_limit', 0, array() ); wp_safe_redirect( $fail ); exit; }
	...
	if ( ! $privacy || ! is_email( $email ) || nvx_valoracion_name_length( $firstname ) < 2 || nvx_valoracion_name_length( $lastname ) < 2 || strlen( $phone ) < 7 || nvx_valoracion_name_length( $message ) < 10 ) { nvx_valoracion_log_outcome( 'FAILURE', 'validation', 0, array() ); wp_safe_redirect( $fail ); exit; }

	$fields = array(
		array( 'objectTypeId' => '0-1', 'name' => 'firstname', 'value' => $firstname ), array( 'objectTypeId' => '0-1', 'name' => 'lastname', 'value' => $lastname ),
		array( 'objectTypeId' => '0-1', 'name' => 'email', 'value' => $email ), array( 'objectTypeId' => '0-1', 'name' => 'phone', 'value' => $phone ),
	...
	if ( $marketing_consent ) {
		...
		$click_id_map = array( 'gclid' => 'nvx_google_click_id', 'gbraid' => 'nvx_google_braid', 'wbraid' => 'nvx_google_wbraid', 'gclsrc' => 'nvx_google_gclsrc' );
		...
		$managed_text = array( 'nvx_first_source','nvx_first_medium','nvx_first_campaign_id','nvx_first_referrer_domain','nvx_first_timestamp','nvx_first_channel','nvx_conversion_channel','nvx_conversion_source','nvx_conversion_medium','nvx_conversion_campaign_id','nvx_conversion_timestamp','nvx_attribution_captured_at','nvx_attribution_expires_at' );
		foreach ( $managed_text as $property ) { $value = isset( $_POST[ $property ] ) ? sanitize_text_field( wp_unslash( (string) $_POST[ $property ] ) ) : ''; nvx_valoracion_append_field( $fields, $property, $value ); }
		foreach ( array( 'nvx_first_landing_url','nvx_conversion_landing_url','nvx_landing_url' ) as $property ) { $value = isset( $_POST[ $property ] ) ? esc_url_raw( wp_unslash( (string) $_POST[ $property ] ) ) : ''; nvx_valoracion_append_field( $fields, $property, $value ); }
	}

	$request_uri = isset( $_SERVER['REQUEST_URI'] ) ? sanitize_text_field( wp_unslash( (string) $_SERVER['REQUEST_URI'] ) ) : '/madrid/valoracion/';
	$request_uri = '/' . trim( $request_uri, '/' );
	$allowed_paths = array( '/madrid/valoracion', '/contacto', '/gracias' );
	$is_allowed = false;
	foreach ( $allowed_paths as $allowed_path ) if ( 0 === strpos( $request_uri, $allowed_path ) ) { $is_allowed = true; break; }
	...
	$context = array( 'pageUri' => $page_uri, 'pageName' => $page_name );
	$hutk_present = 'no';
	if ( $marketing_consent && isset( $_COOKIE['hubspotutk'] ) ) { $hutk = sanitize_text_field( wp_unslash( (string) $_COOKIE['hubspotutk'] ) ); if ( '' !== $hutk ) { $context['hutk'] = $hutk; $hutk_present = 'yes'; } }
	...
	$qa_context = array( 'portal_id' => $portal, 'form_id' => $form, 'pageUri_hash' => substr( md5( $page_uri ), 0, 8 ), 'consent' => $marketing_consent ? 'granted' : 'denied', 'hutk_present' => $hutk_present, 'test_id' => function_exists( 'wp_generate_uuid4' ) ? substr( wp_generate_uuid4(), 0, 8 ) : 'unknown' );
	$result = nvx_valoracion_forward_to_hubspot( $fields, $context );
	if ( $result['ok'] ) { nvx_valoracion_log_outcome( 'SUCCESS', '', $result['status'], $qa_context ); wp_safe_redirect( nvx_valoracion_direct_success_redirect_url() ); exit; }
	nvx_valoracion_log_outcome( 'FAILURE', $result['reason'], $result['status'], $qa_context ); wp_safe_redirect( $fail ); exit;
}
...
function nvx_valoracion_forward_to_hubspot( array $fields, array $context ): array {
	$failed = static function ( string $reason, int $status ): array { return array( 'ok' => false, 'reason' => $reason, 'status' => $status ); };
	$url = function_exists( 'nvx_hubspot_secure_original_url' ) ? nvx_hubspot_secure_original_url() : '';
	if ( '' === $url ) return $failed( 'hubspot_config', 0 );
	$body = wp_json_encode( array( 'fields' => $fields, 'context' => $context, 'legalConsentOptions' => array( 'consent' => array( 'consentToProcess' => true, 'text' => 'Al facilitar tus datos aceptas la Política de privacidad y el tratamiento de mis datos para gestionar esta solicitud.' ) ) ) );
	if ( ! is_string( $body ) ) return $failed( 'hubspot_transport', 0 );
	$response = wp_remote_post( $url, array( 'timeout' => 12, 'headers' => array( 'Content-Type' => 'application/json' ), 'body' => $body ) );
	if ( is_wp_error( $response ) ) return $failed( 'hubspot_transport', 0 );
	$code = (int) wp_remote_retrieve_response_code( $response );
	if ( $code >= 200 && $code < 300 ) return array( 'ok' => true, 'reason' => '', 'status' => $code );
