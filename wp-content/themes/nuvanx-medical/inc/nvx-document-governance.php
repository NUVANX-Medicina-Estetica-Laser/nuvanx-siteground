...
function nvx_document_governance_bind_governed_blog_query( array $query_vars ): array {
	...
	unset( $query_vars['pagename'], $query_vars['page_id'], $query_vars['attachment'], $query_vars['attachment_id'] );
...
/** Resolve governed journal metadata by the requested slug. */
...
/** Preserve an exact governed blog route from cross-post canonical redirects. */
...
function nvx_document_governance_is_approved_strategy_route(): bool {
	...
	return null !== $key && 'approved_for_publication' === ( $catalog[ $key ]['review_status'] ?? null );
}
...
function nvx_document_governance_production_public_url(): string {
	...
	$path = function_exists( 'nvx_seo_current_path' ) ? nvx_seo_current_path() : nvx_document_governance_request_path();
	return home_url( $path );
}

/** Open Graph URL on non-production points at production for approved routes. */
...
/** Resolve canonical URL without changing staging robots policy. */
...
function nvx_document_governance_enqueue_assets(): void {
	...
	$config = array( 'mobileNavId' => 'nvx-mobile-nav' );
	$encoded = wp_json_encode( $config, JSON_UNESCAPED_SLASHES | JSON_HEX_TAG | JSON_HEX_AMP );
	if ( ! is_string( $encoded ) ) {
		$encoded = '{}';
	}
	wp_add_inline_script( 'nvx-runtime-governance', 'window.nvxRuntimeGovernance=' . $encoded . ';', 'before' );
}
